...
if [ "$SCREEN" = "all" ]; then
  ...
  SCREENS=(signin backfill today today-edit today-sheet today-sheet-vitals today-sheet-steps today-sheet-muscle today-sheet-records today-weighin today-board train train-done train-pending train-cardio train-empty train-monday train-past train-past-open train-customize train-customized mini-player logger logger-stats logger-lifts logger-paused logger-finish logger-timer set-row set-row-split set-row-cardio set-row-records set-options effort-picker day day-rows day-past day-session day-two day-empty day-stress day-soreness day-hero sleep-edit stress stress-log stress-day fatigue quick-log scale scale-first day-swap doms stack stack-add fuel fuel-over fuel-empty nutrients macro-edit you levers sync-status sync-doctor plan body volume library exercise reports report report-edit history history-week session session-ledger session-records session-pairs session-cardio exercise-history trends trends-empty body-trends body-trends-empty body-trends-stress appearance appearance-locked widgets)
fi
