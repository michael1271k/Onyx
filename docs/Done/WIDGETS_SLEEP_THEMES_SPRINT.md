...
- **`WarmupCardio.name` survives.** `ExerciseIndexTests` pins `ExerciseSlug.id(...)` to the treadmill slug and the founder's uploaded treadmill sets are filed under it. It is documented as the legacy slug anchor and is no longer the opener's name.
