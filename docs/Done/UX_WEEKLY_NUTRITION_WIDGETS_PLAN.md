...
12. **Native only.** The predecessor web app is untouched; it has no wrap surface at
...
Capacitor plugin (the predecessor's `ios/App/App/` Health bridge). There is no
...
(the predecessor's `ios/App/App/` Health bridge):
...
- the predecessor's `ios/App/App/` Health bridge — `self.store.requestAuthorization(toShare: nil, read: types)`
