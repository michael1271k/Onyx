...
| Wave | Title | Model | Version shipped |
|---|---|---|---|
...
| W5 | Post-workout telemetry, Health provenance, Hevy compare | **Fable 5.1 (High)** | 7.6.0 |
| W6 | Lean: profiling, main-thread purge, compactification, gym mode | Opus 5 (Extra High) | 7.7.0 |
| W7 | AI export: gate, App Intent, MCP server, paste-back ingest | Opus 5 (Extra High) | 7.8.0 |
| W8 | Purge & merge | Opus 5 (Extra High) | 7.8.1 |

> The four numbers above are what each wave shipped, not what this table first
> predicted. 7.4.0 and 7.5.0 went to the report overhaul and export v6 on `main`
> between W4 and W5, and every wave after that moved up. `package.json` is the
> source of truth; this column is a record.
...
### Wave 8 Summary — 7.8.1

**Shipped as 7.8.1, not the plan's 7.6.1.** 7.6.1 would have been a downgrade:
`main` was at 7.8.0. The wave map's version column has been corrected to what
each wave actually shipped and relabelled a record rather than a prediction.

**Worked.** Ten merged branches gone (`-d`, never `-D`; none existed on
`origin`), one worktree, `main` alone. 95.8 GB of derived data deleted —
thirty-three dead per-wave trees under `~/Library/Caches/onyx-swift/`, 103 G →
7.2 G — plus `native/graphify-out/` (108 MB, regrown since the last purge),
`__screenshots__` (146 MB), `__store__`, the `.xcodeproj`, three stale graphify
rollbacks and `tools/onyx-mcp/node_modules`. Six cache paths kept, because the
gates name them. This file's body is now the eight summaries, each in its own
wave section; the prompts are Appendix B.

**Deleting `docs/sql/` is not a deletion, it is a move.** The three applied
files were cited by path from ELEVEN places in live code, one of them a 404
hint `server.mjs` PRINTS as a fix instruction. Their text is now in
`docs/CHANGELOG.md` § *Appendix — applied server migrations* and all eleven
references point there. One of the eleven already pointed at
`w3-sleep-onset.sql`, which the PREVIOUS sprint's close-out deleted: three
close-outs in a row have produced this orphan, and the appendix is the fix.

**The close-out found a crash, and did not close around it.**
`PrescriptionPaste.firstInteger` was `firstDouble(text).map { Int($0) }`;
`firstDouble` accumulates an unbounded digit run and `Int.init(Double)` TRAPS.
`PrescriptionsView` calls `parse` inside its `body`, so a pasted table with a
twenty-digit cell killed the app on the keystroke that pasted it. Proved by
reverting the fix: the test process dies with `Double value cannot be converted
to Int`. **The sprint had clamped this exact trap in two other files — W7's
`TargetsApply.whole` even carries a comment explaining it — and missed the
third.** A review that only reads the diff cannot find the sibling the diff did
not touch. Also fixed: `figure` rounded half-to-even while `whole` rounded
half-away-from-zero, so an exact `.5` showed 2,100 and stored 2101, and with no
other field moved the diff row was suppressed *while the write still happened*.

**The gate is load-sensitive and said so.** `swift:data` reported 4 issues on
the run that shared the machine with two review agents, and 727/727 twice in
isolation — W7's documented `SeamBenchmarkTests` flake. Read it twice before
believing it. OnyxTests: 11 issues / 9 names, the baseline, unchanged by the
two fixes.

**Left open.** (1) `ExportRange.clamp` truncates "since last export" to 28 days
but still advances the marker to the span end, so a device that last exported
months ago loses the gap permanently — a product decision, left to the founder.
(2) `.git` is 930 MB and no close-out has ever touched it. (3) The six kept
cache paths will grow again; `docs/SIMULATORS.md` now names them and tells the
next wave to delete its own `SHOT_DERIVED`, which is the only thing that stops
this recurring.
...
## Appendix B — prompts as run

The eight prompts exactly as they were handed to their agents, each in the
state it was in when that wave started. They are kept because a wave summary
says what happened and only the prompt says what was asked for — and five of
the eight summaries begin by recording that the prompt was wrong about
something. Collapsed, because nothing here is meant to be read straight
through.

<details>
<summary><b>W1 prompt</b></summary>
